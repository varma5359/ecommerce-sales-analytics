...
def make_copy(df):
    ...
    Why?
    ----
    Cleaning should not modify the original raw dataset.
...
def remove_duplicates(df):
    ...
    Note:
    -----
    This rule is specific to this sales project because
...
def fill_missing_text(df):
    """
    Replace missing values in configured text columns
    with 'Unknown'.
...
def remove_missing_critical(df):
    ...
    Important:
    ----------
    We first check whether the expected columns actually exist.

    Missing schema columns should normally be handled by
    ...
    # Find which critical columns are missing completely
    missing_columns = [
        col
...
def remove_invalid_rows(df):
    ...
    if removed > 0:
        ...
        print(
            f"Removed {removed} invalid row(s)."
        )
...
def add_new_columns(df):
    ...
    if (
        "gross_sales" in df.columns
        and "discount" in df.columns
    ):
    ...
    if (
        "gross_sales" in df.columns
        and "discount_amount" in df.columns
    ):
